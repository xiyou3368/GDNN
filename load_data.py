...
def load_nci(data_index):
  dataset = load_protein_dataset(data_index)
  cv_index =2
  ad_train_list = []
  ad_test_list = []
  fea_train_list = []
  ...
  for train_index, test_index in kf.split(dataset[0]):
    if j == cv_index:
      ...
      for item in dataset[2]:
        ...
        i += 1
    j += 1
  return ad_train_list+ad_test_list,fea_train_list+fea_test_list,ad_test_list,fea_test_list,label_train_list+label_test_list,label_test_list 

...
def preprocess_adj(adj):
    ...
    true_a = adj_normalized + 0.5 * sym_l.dot(sym_l) - 1.0/6 * sym_l.dot(sym_l).dot(sym_l) + 1.0/24 * sym_l.dot(sym_l).dot(sym_l).dot(sym_l)
...
def construct_feed_dict(features, support,inverse_support,labels, num_nodes,g_l,placeholders):
    """Construct feed dictionary."""
    feed_dict = dict()
    feed_dict.update({placeholders['labels']: labels})
    feed_dict.update({placeholders['g_labels']: g_l})
...
def re_order(tuopu_list,feature_list,y_label):
    ...
    return tuopu_input,word_input,y_input,index_result
